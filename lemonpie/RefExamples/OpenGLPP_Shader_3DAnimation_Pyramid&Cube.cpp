...
GLuint gVbo_color;
// no need of separate vbos but just for understanding, one should suffice in future
...
void initialize(void)
{
	...
	//glEnable(GL_CULL_FACE); // switch it off for 3D
...
void display(void)
{
	...
	mat4 rotationMatrix = mat4::identity(); // for animation
	...
	// 12 for four faces with three vertices each
									  // *** unbind vao ***
	glBindVertexArray(0);

	//// ******************* DRAW SQARE ******************
	// do load identity first
	...
	// rotate cube
	rotationMatrix = rotate(angleCube, angleCube, angleCube); // polymorphism of rotate()
	modelViewProjectionMatrix = modelViewProjectionMatrix * rotationMatrix;
	...
	// *** draw, either by glDrawTriangles() or glDrawArrays() or glDrawElements()
	glDrawArrays(GL_TRIANGLE_FAN, 0, 4); // 3 (each with its x,y,z )
